...
static struct gpio_keys_button p750_button_table[] = {
	...
	{KEY_CAMERA - 1, GPIO86_P750_BUTTON_CAMERA_HALF, 0, "Camera focus",
	 EV_KEY},
	{KEY_CAMERA, GPIO101_P750_BUTTON_CAMERA_FULL, 0, "Camera button",
	 EV_KEY},
//      {KEY_PLAY,      GPIO_P750_BUTTON_HEADPHONE,     0, "Headset button",    EV_KEY},
	{SW_HEADPHONE_INSERT, GPIO14_P750_HEADPHONE_IN, 0,
	 "Headset insert sw", EV_SW},
};
...
/* GPIO Joystick */
/******************************************************************************/
static struct gpio_joystick_platform_data p750_gpio_joystick_pdata = {
...
/* USB cable and AC adaptor detect init */
/******************************************************************************/
...
static int p750_usb_detect_init(struct device *dev)
{
	int rc;
	int irq = gpio_to_irq(GPIO12_P750_USB_CABLE_DETECT);

	/* enable RE/FE interrupt on cable insertion and removal */
	rc = request_irq(irq, usb_detect_handler,
			 IRQF_DISABLED | IRQF_TRIGGER_RISING |
...
static void p750_setcharge(int flags)
{
	switch(flags) {
	case PDA_POWER_CHARGE_USB:
		gpio_set_value(EGPIO_P750_CHARGE_FAST, 0);
		gpio_set_value(EGPIO_P750_CHARGE_EN1, 1);
		gpio_set_value(EGPIO_P750_CHARGE_EN2_N, 0);
		break;
	case PDA_POWER_CHARGE_AC:
		gpio_set_value(EGPIO_P750_CHARGE_FAST, 1);
		gpio_set_value(EGPIO_P750_CHARGE_EN1, 1);
		gpio_set_value(EGPIO_P750_CHARGE_EN2_N, 0);
		break;
	default:
		/* charging off */
		gpio_set_value(EGPIO_P750_CHARGE_FAST, 0);
		gpio_set_value(EGPIO_P750_CHARGE_EN1, 0);
		gpio_set_value(EGPIO_P750_CHARGE_EN2_N, 1);
	}
}

static struct pda_power_pdata power_supply_info = {
	.init = p750_usb_detect_init,
...
P750_SIMPLE_DEV(p750_gpio_joystick, "gpio-joystick", &p750_gpio_joystick_pdata);
...
static struct pxaohci_platform_data p750_ohci_info = {
	...
	.flags = ENABLE_PORT1 | POWER_CONTROL_LOW /*FIXME*/,
	.power_budget = 0,
};

struct gpio_ress p750_global_gpios[] = {
	P750_GPIO_IN(GPIO12_P750_USB_CABLE_DETECT, "USB detect"),
	P750_GPIO_IN(GPIO19_P750_AC_ADAPTER_DETECT, "AC detect"),
	...
	P750_GPIO_IN(GPIO113_P750_TOUCH_IRQ, "Touch IRQ"),
};
...
static void __init p750_init(void)
{
	...
	spi_register_board_info(p750_spi_board_info,
				ARRAY_SIZE(p750_spi_board_info));
