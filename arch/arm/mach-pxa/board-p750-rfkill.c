...
static int p750_allocate_rfkill(struct device *dev, struct rfkill *rfkdev,
				char *name, enum rfkill_type type, void *toggle_func)
{
	int rc;
	...
	rfkdev->user_claim_unsupported = 0;
	rfkdev->user_claim = 1;
...
static int __init p750_rfkill_probe(struct platform_device *pdev)
{
	...
	rc = p750_allocate_rfkill(&(pdev->dev), bt_rfk, "bluetooth",
				RFKILL_TYPE_BLUETOOTH, bluetooth_set_power);
 	rc |= p750_allocate_rfkill(&pdev->dev, wifi_rfk, "wifi",
 				RFKILL_TYPE_WLAN, wifi_set_power);
 	rc |= p750_allocate_rfkill(&pdev->dev, gps_rfk, "gps",
 				RFKILL_TYPE_UWB, gps_set_power);
 	rc |= p750_allocate_rfkill(&pdev->dev, gsm_rfk, "gsm",
